...
int main() {
    ...
    cin >> n;
    vector<vector<pair<int, vector<pair<int, int>>>>> x(n, vector<pair<int, vector<pair<int, int>>>>(m));
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            if (a[j] == '.') {
                x[i][j] = make_pair(-1, vector<pair<int,int>>());
                continue;
            }
            if (a[j] == '0') {
            ...
            if (a[j] == '9') {
                ...
                xs.push_back(pos);
                x[i][j] = make_pair(int(a[j])-48, vector<pair<int,int>>(xs));
            }
            else {
                x[i][j] = make_pair(int(a[j])-48, vector<pair<int,int>>());
            }
    ...
    for (int i = 0; i < zeros.size(); i++) {
        //dfs traversal
        sum += dfs(x, zeros[i].first, zeros[i].second).size();
    }
    //printss(x);
